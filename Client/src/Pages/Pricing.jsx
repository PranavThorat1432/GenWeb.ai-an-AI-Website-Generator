...
const Pricing = () => {
    ...
    const handleBuy = async (planKey) => {
        ...
        if(planKey === 'free' || userData.credits < 100) {
            navigate('/');
            return;
        }
        setLoading(planKey);
        ...
        try {
            const result = await axios.post(`${serverUrl}/api/billing`, {planType: planKey}, {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#050505] text-white px-6 pt-16 pb-24">
        <div className="absolute inset-0 pointers-event-none">
        ...
        <button className="relative z-10 mb-8 flex items-center gap-2 text-sm text-zinc-400 hover:text-white transition cursor-pointer ease-in-out duration-200" onClick={() => navigate('/')}>
            ...
            {plans.map((p, i) => (
                ...
                >
                    ...
                    <motion.button
                        ...
                        className={`w-full py-3 rounded-xl font-semibold transition cursor-pointer mt-5
                            ${p.popular
